...
Jag har även fått använda och blivit van med bl.a. `useSearchParams` och `useParams` (och skillnaden mellan dem!), `useNavigate`, `useLocation()` för att cacha API data i en liten "ryggsäck" för att minska antalet requests haha, optional chaining för att säkert nå och använda API data, och `useRef()` för att skapa en referens till en lokalt scoped useEffect variabel som i en annan funktion!  
